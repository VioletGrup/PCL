...
def test_grade_project_with_floats():
    """
    Test that the API accepts float pile IDs (e.g., 1.1) and returns them as normalized strings ("1.10").
    """
    request_data = {
        "tracker_type": "flat",
        "piles": [
            {
                "pile_id": 1.01,  # Float input
                "pile_in_tracker": 1,
                "northing": 0.0,
                "easting": 0.0,
                "initial_elevation": 100.0,
            ...
            {
                "pile_id": 1.1,  # Float input (should become "1.10")
                "pile_in_tracker": 10,
                "northing": 10.0,
                "easting": 0.0,
                "initial_elevation": 100.5,
    ...
    assert "1.01" in piles
    assert "1.10" in piles  # 1.1 float -> "1.10" string
